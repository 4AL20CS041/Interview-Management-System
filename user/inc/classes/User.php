...
class User{
  ...
  public function userRegistration($data){
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['register-submit'])) {
      ...
      if (filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
        ...
        $sql = "select client_email from client where client_email = ?";
        $arr = array($email);
      ...
      $sql = "insert into client(client_name, client_email, client_pass) values(?, ?, ?)";
  ...
  public function userLogin($data){
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['login-submit'])) {
      ...
      $sql = "select * from client where client_email = ? AND client_pass = ?";
      $arr = array($email, $md5password);
      $simplequery = $this->db->simplequery($sql, $arr);
      $results = $simplequery->fetch(PDO::FETCH_OBJ);
      if ($results) {
        $userSession = new Session();
        $userSession->setSession('clientlogin', true);
        $userSession->setSession('client_id', $results->user_id);
        $userSession->setSession('client_name', $results->user_name);
        $userSession->setSession('client_email', $results->email);
